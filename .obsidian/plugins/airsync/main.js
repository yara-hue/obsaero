...
var AirsyncSettingTab = class extends import_obsidian.PluginSettingTab {
  constructor(app, plugin) {
    super(app, plugin);
    this.connectBtn = null;
    this.connectBtnEl = null;
    this.plugin = plugin;
  ...
  display() {
    ...
    new import_obsidian.Setting(containerEl).setName("API Key").setDesc("Firebase Web API Key").addText(
      (text2) => text2.setPlaceholder("AIzaSy...").setValue(this.plugin.settings.firebaseApiKey).onChange(async (val) => {
        this.plugin.settings.firebaseApiKey = val;
        await this.plugin.saveSettings();
        this.updateConnectButton();
      })
    );
    new import_obsidian.Setting(containerEl).setName("Auth Domain").setDesc("Firebase Auth Domain").addText(
      (text2) => text2.setPlaceholder("your-project.firebaseapp.com").setValue(this.plugin.settings.firebaseAuthDomain).onChange(async (val) => {
        this.plugin.settings.firebaseAuthDomain = val;
        await this.plugin.saveSettings();
        this.updateConnectButton();
      })
    );
    new import_obsidian.Setting(containerEl).setName("Database URL").setDesc("Firebase Realtime Database URL").addText(
      (text2) => text2.setPlaceholder("https://your-project-default-rtdb.firebaseio.com").setValue(this.plugin.settings.firebaseDatabaseURL).onChange(async (val) => {
        this.plugin.settings.firebaseDatabaseURL = val;
        await this.plugin.saveSettings();
        this.updateConnectButton();
      })
    );
    new import_obsidian.Setting(containerEl).setName("Project ID").setDesc("Firebase Project ID").addText(
      (text2) => text2.setPlaceholder("your-project").setValue(this.plugin.settings.firebaseProjectId).onChange(async (val) => {
    ...
    this.connectBtn = new import_obsidian.Setting(containerEl).setName("Connection").setDesc(this.getConnectionDesc()).addButton((btn) => {
      btn.onClick(async () => {
    ...
    new import_obsidian.Setting(containerEl).setName("Display Name").setDesc("Your name shown to collaborators").addText(
      (text2) => text2.setPlaceholder("Enter your display name").setValue(this.plugin.settings.displayName).onChange(async (val) => {
  ...
  updateButtonState(btn) {
    switch (this.plugin.connectionState) {
      case "disconnected": {
        const hasConfig = !!this.plugin.settings.firebaseApiKey && !!this.plugin.settings.firebaseAuthDomain && !!this.plugin.settings.firebaseDatabaseURL && !!this.plugin.settings.firebaseProjectId;
        btn.textContent = "Connect";
        btn.className = hasConfig ? "mod-cta" : "";
...
function updateEmulatorBanner(name5, isRunningEmulator) {
  ...
  function tearDown() {
    const element2 = document.getElementById(bannerId);
    if (element2) {
      element2.remove();
    }
  }
  ...
  for (const [key, value] of Object.entries(querystringParams)) {
    if (Array.isArray(value)) {
      value.forEach((arrayVal) => {
        params2.push(encodeURIComponent(key) + "=" + encodeURIComponent(arrayVal));
      });
    } else {
      params2.push(encodeURIComponent(key) + "=" + encodeURIComponent(value));
    }
  }
  return params2.length ? "&" + params2.join("&") : "";
}
...
var defaultLogHandler = (instance, logType, ...args2) => {
  ...
  if (method) {
    console[method](`[${now}]  ${instance.name}:`, ...args2);
  } else {
...
var Logger = class {
   ...
   */
  debug(...args2) {
    this._userLogHandler && this._userLogHandler(this, LogLevel.DEBUG, ...args2);
    this._logHandler(this, LogLevel.DEBUG, ...args2);
  }
  log(...args2) {
    this._userLogHandler && this._userLogHandler(this, LogLevel.VERBOSE, ...args2);
    this._logHandler(this, LogLevel.VERBOSE, ...args2);
  }
  info(...args2) {
    this._userLogHandler && this._userLogHandler(this, LogLevel.INFO, ...args2);
    this._logHandler(this, LogLevel.INFO, ...args2);
  }
  warn(...args2) {
    this._userLogHandler && this._userLogHandler(this, LogLevel.WARN, ...args2);
    this._logHandler(this, LogLevel.WARN, ...args2);
  }
  error(...args2) {
    this._userLogHandler && this._userLogHandler(this, LogLevel.ERROR, ...args2);
    this._logHandler(this, LogLevel.ERROR, ...args2);
  }
};
...
function _logWarn(msg, ...args2) {
  if (logClient.logLevel <= LogLevel.WARN) {
    logClient.warn(`Auth (${SDK_VERSION}): ${msg}`, ...args2);
  }
}
function _logError(msg, ...args2) {
  if (logClient.logLevel <= LogLevel.ERROR) {
    logClient.error(`Auth (${SDK_VERSION}): ${msg}`, ...args2);
  }
}
...
async function _performApiRequest(auth, method, path, request, customErrorMap = {}) {
  return _performFetchWithErrorHandling(auth, customErrorMap, async () => {
    ...
    const query = querystring(Object.assign({ key: auth.config.apiKey }, params2)).slice(1);
...
var OAuthCredential = class _OAuthCredential extends AuthCredential {
  ...
  /** @internal */
  static _fromParams(params2) {
    const cred = new _OAuthCredential(params2.providerId, params2.signInMethod);
    if (params2.idToken || params2.accessToken) {
      if (params2.idToken) {
        cred.idToken = params2.idToken;
      }
      if (params2.accessToken) {
        cred.accessToken = params2.accessToken;
      }
      if (params2.nonce && !params2.pendingToken) {
        cred.nonce = params2.nonce;
      }
      if (params2.pendingToken) {
        cred.pendingToken = params2.pendingToken;
      }
    } else if (params2.oauthToken && params2.oauthTokenSecret) {
      cred.accessToken = params2.oauthToken;
      cred.secret = params2.oauthTokenSecret;
    } else {
      _fail(
        "argument-error"
        /* AuthErrorCode.ARGUMENT_ERROR */
...
var UserCredentialImpl = class _UserCredentialImpl {
  constructor(params2) {
    this.user = params2.user;
    this.providerId = params2.providerId;
    this._tokenResponse = params2._tokenResponse;
    this.operationType = params2.operationType;
  }
...
function _signIn(params2) {
  return _signInWithCredential(params2.auth, new IdpCredential(params2), params2.bypassAuthState);
}
function _reauth(params2) {
  const { auth, user } = params2;
  _assert(
    user,
    auth,
    "internal-error"
  ...
  return _reauthenticate(user, new IdpCredential(params2), params2.bypassAuthState);
}
async function _link(params2) {
  const { auth, user } = params2;
  _assert(
    user,
    auth,
  ...
  return _link$1(user, new IdpCredential(params2), params2.bypassAuthState);
}
var AbstractPopupRedirectOperation = class {
  ...
  async onAuthEvent(event) {
    ...
    try {
      this.resolve(await this.getIdpTask(type)(params2));
    } catch (e) {
      this.reject(e);
    }
  }
...
function getIframeUrl(auth) {
  ...
  if (frameworks.length) {
    params2.fw = frameworks.join(",");
  }
  return `${url}?${querystring(params2).slice(1)}`;
}
...
async function _getRedirectUrl(auth, provider, authType, redirectUrl, eventId, additionalParams) {
  ...
  if (provider instanceof FederatedAuthProvider) {
    ...
    params2.providerId = provider.providerId || "";
    if (!isEmpty(provider.getCustomParameters())) {
      params2.customParameters = JSON.stringify(provider.getCustomParameters());
    }
  ...
  if (provider instanceof BaseOAuthProvider) {
    ...
    if (scopes.length > 0) {
      params2.scopes = scopes.join(",");
    }
  }
  if (auth.tenantId) {
    params2.tid = auth.tenantId;
  }
  const paramsDict = params2;
...
var ObjectToUniqueKey = function(obj) {
  ...
  for (let i = 0; i < keys2.length; i++) {
    ...
    key += stringify(keys2[i]);
    key += ":";
    key += ObjectToUniqueKey(obj[keys2[i]]);
  }
  key += "}";
  return key;
};
...
function repoInfoConnectionURL(repoInfo, type, params2) {
  assert(typeof type === "string", "typeof type must == string");
  assert(typeof params2 === "object", "typeof params must == object");
  let connURL;
  if (type === WEBSOCKET) {
  ...
  if (repoInfoNeedsQueryParam(repoInfo)) {
    params2["ns"] = repoInfo.namespace;
  }
  const pairs = [];
  each(params2, (key, value) => {
...
var BrowserPollConnection = class _BrowserPollConnection {
  ...
  constructor(connId, repoInfo, applicationId, appCheckToken, authToken, transportSessionId, lastSessionId) {
    ...
    this.urlFn = (params2) => {
      if (this.appCheckToken) {
        params2[APP_CHECK_TOKEN_PARAM] = this.appCheckToken;
      }
      return repoInfoConnectionURL(repoInfo, LONG_POLLING, params2);
    };
  }
  /**
  ...
  open(onMessage, onDisconnect) {
    ...
    executeWhenDOMReady(() => {
      ...
      this.scriptTagHolder = new FirebaseIFrameScriptHolder((...args2) => {
        const [command, arg1, arg2, arg3, arg4] = args2;
        this.incrementIncomingBytes_(args2);
   ...
   */
  incrementIncomingBytes_(args2) {
    const bytesReceived = stringify(args2).length;
...
var RangedFilter = class _RangedFilter {
  constructor(params2) {
    this.indexedFilter_ = new IndexedFilter(params2.getIndex());
    this.index_ = params2.getIndex();
    this.startPost_ = _RangedFilter.getStartPost_(params2);
    this.endPost_ = _RangedFilter.getEndPost_(params2);
    this.startIsInclusive_ = !params2.startAfterSet_;
    this.endIsInclusive_ = !params2.endBeforeSet_;
  }
  getStartPost() {
  ...
  static getStartPost_(params2) {
    if (params2.hasStart()) {
      const startName = params2.getIndexStartName();
      return params2.getIndex().makePost(params2.getIndexStartValue(), startName);
    } else {
      return params2.getIndex().minPost();
    }
  }
  static getEndPost_(params2) {
    if (params2.hasEnd()) {
      ...
      return params2.getIndex().maxPost();
    }
  }
};
...
var View = class {
  constructor(query_, initialViewCache) {
    ...
    const params2 = this.query_._queryParams;
    const indexFilter = new IndexedFilter(params2.getIndex());
    const filter = queryParamsGetNodeFilter(params2);
...
function syncPointViewForQuery(syncPoint, query) {
  const params2 = query._queryParams;
  if (params2.loadsAllData()) {
...
function push(parent, value) {
  ...
  const thenablePushRef = child(parent, name5);
...
function assignColor(uid, index = 0) {
...
var FirebaseService = class {
  ...
  async saveUserProfile(uid, displayName) {
    if (!this.db) return;
  ...
  getUserId() {
    var _a, _b;
    return (_b = (_a = this.user) == null ? void 0 : _a.uid) != null ? _b : null;
  }
};
...
var ObservableV2 = class {
   ...
   */
  off(name5, f) {
    const observers = this._observers.get(name5);
    if (observers !== void 0) {
      observers.delete(f);
      if (observers.size === 0) {
        this._observers.delete(name5);
      }
    }
  }
  /**
   ...
   */
  emit(name5, args2) {
    return from((this._observers.get(name5) || create()).values()).forEach((f) => f(...args2));
  }
  destroy() {
    this._observers = create();
  }
...
var hasParam = (name5) => computeParams().has(name5);
var getVariable = (name5) => isNode ? undefinedToNull(process.env[name5.toUpperCase().replaceAll("-", "_")]) : undefinedToNull(varStorage.getItem(name5));
var hasConf = (name5) => hasParam("--" + name5) || getVariable(name5) !== null;
...
var createElement = (name5) => doc.createElement(name5);
...
var element = (name5, attrs = [], children = []) => append(setAttributes(createElement(name5), attrs), children);
...
var appendChild = (parent, child2) => parent.appendChild(child2);
...
var Doc = class _Doc extends ObservableV2 {
   ...
   * @public
   */
  get(name5, TypeConstructor = (
    /** @type {any} */
    AbstractType
  )) {
    const type = setIfUndefined(this.share, name5, () => {
    ...
    if (TypeConstructor !== AbstractType && Constr !== TypeConstructor) {
      if (Constr === AbstractType) {
        ...
        throw new Error(`Type with the name ${name5} has already been defined with a different constructor`);
      }
    }
    return (
      /** @type {InstanceType<Type>} */
   ...
   * @public
   */
  getXmlFragment(name5 = "") {
    return this.get(name5, YXmlFragment);
  }
  /**
...
var isParentOf = (parent, child2) => {
  while (child2 !== null) {
    if (child2.parent === parent) {
      return true;
    }
    child2 = /** @type {AbstractType<any>} */
    child2.parent._item;
  }
  return false;
};
...
var getPathTo = (parent, child2) => {
  const path = [];
  while (child2._item !== null && child2 !== parent) {
    if (child2._item.parentSub !== null) {
      path.unshift(child2._item.parentSub);
    } else {
      let i = 0;
      let c = (
        /** @type {AbstractType<any>} */
        child2._item.parent._start
      );
      while (c !== child2._item && c !== null) {
...
var YRemoteSelectionsPluginValue = class {
  ...
  update(update) {
    ...
    awareness.getStates().forEach((state, clientid) => {
      ...
      const { color = "#30bced", name: name5 = "Anonymous" } = state.user || {};
      ...
      decorations.push({
        ...
        value: cmView2.Decoration.widget({
          ...
          block: false,
          widget: new YRemoteCaretWidget(color, name5)
        })
      });
    });
...
// src/main.ts
var AirsyncPlugin = class extends import_obsidian2.Plugin {
  constructor() {
    super(...arguments);
  ...
  async tryInitializeFirebase() {
    if (this.connectionState !== "disconnected") return;
    if (!this.settings.firebaseApiKey || !this.settings.firebaseAuthDomain || !this.settings.firebaseDatabaseURL || !this.settings.firebaseProjectId) {
      new import_obsidian2.Notice("Airsync: Configure Firebase settings first");
      return;
    }
  ...
  async disconnectFirebase() {
    var _a;
    (_a = this.collaboration) == null ? void 0 : _a.destroy();
    try {
    ...
    this.setConnectionState("disconnected");
    new import_obsidian2.Notice("Airsync: Disconnected");
  }
  ...
  updateStatusBar() {
    ...
    dot.style.cssText = "width:8px;height:8px;border-radius:50%;display:inline-block;margin-right:6px;";
...
var DisplayNameModal = class extends import_obsidian2.Modal {
  constructor(app, plugin) {
    super(app);
    this.plugin = plugin;
  }
  onOpen() {
    ...
    submitBtn.onclick = async () => {
      const name5 = this.nameInput.value.trim();
      if (!name5) {
        new import_obsidian2.Notice("Please enter a display name");
        return;
      }
      this.plugin.settings.displayName = name5;
      await this.plugin.saveSettings();
      const uid = this.plugin.firebase.getUserId();
      if (uid) {
        await this.plugin.firebase.saveUserProfile(uid, name5);
      }
      new import_obsidian2.Notice(`Airsync: Connected as ${name5}`);
